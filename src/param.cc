...
_myOwner(owner), _myUpdater(updater),_isTemporal(0), _hasMultipleArgs(0)
...
_myUpdater(0),_hasMultipleArgs(P._hasMultipleArgs)
{
...
string Param::checkArgumentForExpansion (string arg)
{
  ...
  if (arg[0] == '@') { //it is assumed here that this does not belong to a temporal tag
    expanded = getArgumentFromFile(tstring::removeFirstCharOf(arg, '@'));
  } else
    expanded = arg;
...
{
  StreamParser Parser(file.c_str());
  int dummyCnt;
  string arg;
